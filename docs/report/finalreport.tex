...
Choose $\theta$ $\sim$ Dirichlet($\alpha$)\;
	...
	Choose word $w_{n}$ from $p(w_{n} | z_{n}, \beta)$, a multinomial
	probability conditioned on topic $z_{n}$
	}
}
\end{algorithm}

Figure~\ref{lda-model} shows the graphical model representation of LDA, which is
...
by a k X V matrix $\beta$, each row of which represents the distribution of
topics over words in the vocabulary. Also, $\alpha$ is a k-dimensional vector 
which is a parameter for the Dirichlet random variable $\theta$. Given the
parameters $\alpha$ and $\beta$, the joint distribution of the topic mixture
