...
\section{Conclusions}
Recommender systems work well when the users and movies are modeled with latent 
factors. However, they are limited to in-matrix predictions. 
This shortcoming can be overcome by using additional information about the 
movies for out-matrix recommendations. Our experiments have demonstrated this fact by modeling the latent topic space of movie summaries using Latent Dirichlet Allocation. 
Though the collaborative filtering did not benefit a lot, in terms of training and testing error,
but can now perform out-matrix recommendations with reasonable accuracies.
