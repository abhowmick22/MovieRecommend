...
\begin{figure}[h]
	...
	%\captionsetup[subfigure]{oneside,margin={0cm,1.8cm}}
	\begin{subfigure}[b]{0.45\textwidth}
	\fbox{\includegraphics[width=6.35cm,height=6cm]{lda-tsne.png}}
	\caption{2D plot of feature representations of movies}
	\label{fig:lda-movies}
	\end{subfigure}
	~
	\begin{subfigure}[b]{0.45\textwidth}
	\fbox{\includegraphics[width=6.35cm,height=6cm]{close-movies-short.png}}
...
\section{Conclusions}
Recommender systems work well when the users and movies are modelled with latent 
factors. However, they are limited to in-matrix predictions for their recommendations. 
This shortcoming can be overcome, as seen, by using additional information about the 
movies for out-matrix recommendations. Our experiments have demonstrated this capability
by modeling the topic of movie summaries in low dimensions, using Latent Dirichlet Allocation. 
Though the collaborative filtering did not benefit a lot, in terms of training and testing error
by this side-features but can now perform out-matrix recommendations with reasonable accuracies.
